...
def get_train_val_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True):
    ...
    cifar100_training = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=transform_train)
    # 将训练集80%划分为训练集，20%划分为测试集
    train_ratio = 0.8
    val_ratio = 0.2
    # 计算切分的样本数量
    train_size = int(train_ratio * len(cifar100_training))
    val_size = len(cifar100_training) - train_size
    train_dataset, val_dataset = random_split(cifar100_training, [train_size, val_size])
    # 创建 DataLoader 加载器
    cifar100_training_loader = DataLoader(
        train_dataset, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size, pin_memory=True)
    cifar100_val_loader = DataLoader(
        val_dataset, shuffle=False, num_workers=num_workers, batch_size=int(batch_size/2), pin_memory=True)
