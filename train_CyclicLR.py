...
# torch.cuda.manual_seed(seed)  # 设置PyTorch在GPU上的随机种子
...
def train(epoch):
    ...
    for batch_index, (images, labels) in enumerate(cifar100_training_loader):
        ...
        # 每次训练完求精度,并记录
        _, train_preds = outputs.max(1)
        train_correct += train_preds.eq(labels).sum()
        if args.compare:
            _, train_pred_preds = outputs_lrpred.max(1)
            train_pred_correct += train_pred_preds.eq(labels).sum()
        else:
            train_pred_correct = train_correct

        # LR更新
        if epoch > args.warm:
            train_scheduler.step()
            if args.compare:
                train_scheduler_lrpred.step()
        ...
        if batch_index % 20 == 0:
            print('Training Epoch: {epoch}. iter:{batch_index} [{trained_samples}/{total_samples}]\
                   \tLoss: {:0.4f}\tLoss_lrpred: {:0.4f}\tLR: {:0.6f}\tLR_pred: {:0.6f}'.format(
                loss.item(),
                loss_lrpred.item(),
                optimizer.param_groups[0]['lr'],
                optimizer_lrpred.param_groups[0]['lr'],
                epoch=epoch,
    ...
    # 记录训练精度数据
    lengthoftraindataset = len(cifar100_training_loader.dataset)
    acc_train = train_correct.float() / lengthoftraindataset
    acc_pred_train = train_pred_correct.float() / lengthoftraindataset
    writer.add_scalar('train acc/train', acc_train, epoch)
    writer.add_scalar('train acc/train_pred', acc_pred_train, epoch)
    writer.add_scalar('train acc/pred-train', acc_pred_train - acc_train, epoch)
...
if __name__ == '__main__':
    ...
    train_scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.001, max_lr=0.002, step_size_up=1000)
    ...
    if args.compare:
        ...
        train_scheduler_lrpred = torch.optim.lr_scheduler.CyclicLR(optimizer_lrpred, base_lr=0.001, max_lr=0.002, step_size_up=1000)
